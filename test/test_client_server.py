...
import time
import pytest
from pyspec.server import Server
...
# @pytest.mark.asyncio
# async def test_function_execution(server_process):
#     async with ClientConnection(HOST, PORT) as client:
#         # Only works if server has a remote function 'sum' defined
#         try:
#             result = await client.remote_func("sum", 1, 2)
#             assert result == 3
#         except Exception:
#             pytest.skip("No remote function 'sum' defined on server.")


# @pytest.mark.asyncio
# async def test_property_read_write(server_process):
#     async with ClientConnection(HOST, PORT) as client:
#         # Only works if server has a property 'foo' defined
#         try:
#             await client.prop_set("foo", 123)
#             value = await client.prop_get("foo")
#             assert value == 123
#         except Exception:
#             pytest.skip("No property 'foo' defined on server.")
