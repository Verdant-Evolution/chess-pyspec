...
- Always use caution with ``allow_remote_code_execution=True`` as it allows arbitrary code execution. If disabled (default), the server only accepts a restricted expression subset for command/function argument resolution: Python literals and collections, bare symbols, and indexed access such as ``NUMBERS[1]`` or ``LOOKUP['alpha']``.
- SPEC macro syntax that is not Python-expression syntax (for example ``umv th 1``) is intentionally rejected in restricted mode.
